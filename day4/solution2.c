...
int main(int argc, char** argv){
	...
	while(1){
		...
		int moves = solveBoard(&curBoard, numbers, 100);

		printf("\n\nBoard #%d:", counter);

		if(moves < slowest){
			printf(" Did not solve in over %d moves\n", slowest);
		}
		else if(moves == 101){
			printf(" Did not solve at all\n");
		}
		else{
			printf(" Solved in over %d moves. Setting lower limit from %d to %d\n", moves, slowest, moves);
			bestBoard = curBoard;
			slowest = moves;
		}
